...
##### (2a) Все вместе (без разбивки по fields) {.tabset .tabset-fade .tabset-pills}
...
```{r, fig.dim = c(8.8, 5)}
p <- data %>% distinct(UT, .keep_all = TRUE) %>% select(year,research_areas,field) %>% 
...
###### Скользящее среднее + реальные значения
...
data %>% 
  ...
  #   ) +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  facet_wrap(~research_areas, nrow = 3, labeller = label_wrap_gen(width=20)) +
...
subplot(style(ggplotly(p1b), showlegend = F), ggplotly(p1a))
subplot(style(ggplotly(p2b), showlegend = F), ggplotly(p2a))
subplot(style(ggplotly(p3b), showlegend = F), ggplotly(p3a))
```
...
p1b <-ggplot(df, aes(x=year, y=n, color=region)) + 
...
annotations = list( 
  list( 
    x = 0.25,  
    y = 1.0,  
    text = paste0('(А) Доля статей по годам'),
    xref = "paper",  
    yref = "paper",  
    xanchor = "center",  
    yanchor = "bottom",  
  ...
  list( 
    x = 0.75,  
    y = 1,  
    text = paste0('(B) Кол-во статей по годам'),
    xref = "paper",  
    yref = "paper",  
    xanchor = "center",  
    yanchor = "bottom",  
...
subplot(style(ggplotly(p1a), showlegend = F), ggplotly(p1b)) %>%  layout(annotations = annotations)
```
...
p <- df %>% mutate(perc=round(perc*100, digits=2)) %>% 
  ggplot(aes(x=year, perc, color=region)) + 
  geom_line(alpha = 0.6) +
  geom_point(alpha = 0.6, size=1) +
  #geom_ma(ma_fun = SMA, n = 3, linetype = "solid") +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
