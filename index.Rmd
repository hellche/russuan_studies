...
df %>% mutate(perc=perc*100) %>% 
  ...
  geom_ma(ma_fun = SMA, n = 3, linetype = "solid") +
  scale_y_continuous(labels = function(x) paste0(x, "%")) +
  facet_grid(research_areas~region, scales = "free") +
  labs(title = "С. Динамика доли (тоже самое что и рис В)",
      subtitle = "светалая линия - реальные значения; скользящее среднее (шаг = 3 года)") +
  theme_dark() +
  #  theme_bw() +
  scale_color_brewer(palette = "Paired") +
  guides(color=guide_legend(override.aes = list(size=5))) + 
  ...
  theme(strip.background = element_rect(fill = NA),
        ...
        strip.text.y = element_text(colour = 'black',size = 8, angle=0, hjust = 0)) +
  theme(axis.title = element_blank(),
        legend.position  = "none",
        legend.title= element_blank()) +
  theme(axis.text.x = element_text(size = 7, angle = 90, vjust=0.5),
