...
> В документе всё постоено на `full_data_precise_original`. Никаких манипуляций с данным не произведено.
> Если исследовательская область выглядит как список, например, `Political Science; Area Studies`, тогда публикация дублируется и считатся как +1 публикация для каждой из областей в списке. 
